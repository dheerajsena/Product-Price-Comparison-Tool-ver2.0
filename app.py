...
def make_report(
    ...
    def match_with_tolerance(source_price, master_price):
        if pd.isna(source_price) or pd.isna(master_price):
            return "N/A"
        return "Match" if abs(source_price - master_price) <= tolerance else "Mismatch"

    # Status columns (simple for filtering)
    merged["Website vs Master"] = merged.apply(
        lambda r: match_with_tolerance(r.get("Variant Price_Website"), r.get("Variant Price_Master")),
        axis=1,
    )
    merged["Marlin vs Master"] = merged.apply(
        lambda r: match_with_tolerance(r.get("Variant Price_Marlin"), r.get("Variant Price_Master")),
        axis=1,
    )

    # Sentence columns (exactly as you described)
    ...
    def mismatch_summary(row):
        ...
        # No mismatches, but could be all N/A
